...
This guide will show you how to transcribe streaming audio using the Smallest AI's Lightning STT model using our WebSocket API. Our Lightning model provides state of the art low latencies (64ms) for TTFT (Time to First Transcript) making it ideal choice for speech to text conversion during live conversations. 

# Real-Time Audio Transcription

The Real-Time API allows you to stream audio data and receive transcription results as the audio is processed. This is ideal for live conversations, voice assistants, and scenarios where you need immediate transcription feedback. In such scenarios, where minimizing latency is of utmost importance, audio is streamed in chunks of a few kilobytes on a live connection.
...
- **`full_transcript`**: Complete transcription text accumulated so far.
- **`is_final`**: Indicates if this is the final transcription for the current segment.
- **`is_last`**: Indicates if this is the last transcription in the session.
- **`language`**: Detected language code (only when `is_final=true`).
