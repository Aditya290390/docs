...
Common issues and solutions when using the Lightning STT WebSocket API.
...
1. **Verify Encoding Parameters**:
   ...
   url.searchParams.append("encoding", "linear16"); // or linear32, alaw, mulaw
...
**Symptoms**: `word_timestamps` not in response

**Possible Causes**:
- Parameter not set in URL
...
1. **Verify Parameter**:
   ...
   url.searchParams.append("word_timestamps", "true"); // Correct
   // Not: url.searchParams.append("word_timestamps", true);
   ```
