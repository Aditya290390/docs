...
- An API key from the Smallest AI [platform](https://atoms.smallest.ai/dashboard/text-to-speech).  
...
def main():
    ...
    # Get available voices for the "lightning-v2" model
    voices = client.get_voices(model="lightning-v2")
    print(f"Available Voices (Model: 'lightning-v2'): {voices}")
...
- `get_voices(model="model_name")`: Retrieves the voices available for a specific model (e.g., "lightning-v2").  
